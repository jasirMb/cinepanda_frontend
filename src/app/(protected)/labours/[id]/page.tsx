...
export default function LabourDetailPage({
  ...
  const byProject = useMemo(() => {
    ...
    for (const l of logs) {
      const pid =
        typeof l.projectId === "string" ? l.projectId : l.projectId._id;
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
      <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500 dark:text-slate-400">
        <span className={highlight ? "text-cine-primary" : "text-slate-400"}>
          {icon}
        </span>
        {label}
      </div>
      <p className="mt-1 truncate text-lg font-semibold text-slate-900 dark:text-slate-50">
        {value}
      </p>
      {sub && (
